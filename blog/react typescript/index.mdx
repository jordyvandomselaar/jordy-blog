...
So, the submitting works but nothing is happening. Why? We haven't told `<TodoList />` to use `App's` state yet! Let's get to it. On our `<TodoList />`, add the `todos` property with value `todos` like so:
...
That's it for this tutorial. I hope you enjoyed it and that you've learned how to get started with using TypeScript and React together. Also, I hope I was able to show you the benefits of using Typescript if you weren't sure yet. If you're up for a challenge you can try and implement functionality to finish a to-do or disabling the create button if there is no text inside the to-do input. Good luck!
