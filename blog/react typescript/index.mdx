...
- [Adding some styling](#adding-some-styling)
...
Here's a few things you'll be learning in this tutorial;
...
The autocompletions are really neat as well. Most bigger JavaScript packages are written in TypeScript and thus contain types. What this means is that your editor can warn you if you're using a package in the wrong way and it can tell you about all the parameters a method expects in a package. This also works for components; you can now be warned about using the wrong properties of a component before you even run your app. You'll find this is incredibly useful when refactoring components or methods.
...
Open `src/components/TodoList.tsx`, if you'r using Visual Studio Code you should notice a red line underneath `<TodoListItem />`. It's telling us that we have an error. Uh-oh 🤦️
...
Inside `src/components` create a new file: `TodoForm.tsx`, inside it add the following code;
...
Because React does things a little differently (`className` instead of `class`, `htmlFor` instead of `for`) they've defined a generic interface to account for these changes. They called it `HTMLProps`.

So, what's a "generic"?

A generic is an interface or class that takes another class or interface as a type argument. An example of this is is the `Array` class.

You can tell TypeScript that you're working with an array that contains a certain type of data by defining your variable like so;
...
This is called object destructuring. The `...` is called a [spread operator](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Spread_syntax) this allows us to pass all the properties that our component receives through to another component. _You can imagine that it gets extremely boring constantly passing hundreds of different properties around manually._
...
You can type in the input but submitting the form only seems to refresh te page. Hmmm, let's fix that. How? Glad you asked! By adding state. React ❤️ state.
...
Because explaining how state and declarative rendering works is beyond the scope of this blogpost I recommend you read [this post](https://dev.to/itsjzt/declarative-programming--react-3bh2) about declarative programming.
...
React has one-way data flow. This imposes the following rules:

1. A component can have state and it can send state down to child components
2. Child components cannot change the state of their parents\*

\*_Unless you also pass updater functions down, we'll use those as well, hang in there!_

### Adding state to our components

Because state can only flow down we first have to identify the common ancestor to all the components that require our data. Because we're currently building a super simpel to-do app, that's `app.tsx`.
...
We've just added state to our `App` component. Easy right? We've told `App` that it has state called `todos` and it starts out with an empty `array`. Since we're using TypeScript we can tell `App` what kind of array it is as well!
...
`useState` returns an array containing your state and a setter. Using array destructuring you can provide names for both your state and the setter method.

Let's also create state for the current value of our input, this makes adding the to-do later easier.
...
This remove all existing todo's and add one new todo.
...
Are you updating state based on the previous state? Yes? Use the callback. No? Specify the new value.
...
So, the submitting works but nothings happening. Why? We haven't told `<TodoList />` to use `App`'s state yet! Let's get to it. On our `<TodoList />`, add the `todos` property with value `todos` like so:
...
And add the property to the component as well:
...
You should see our "Finish writing this blogpost" to-do disappear. We're now officially rendering our to-do's from state! 🎉️️️️️🎉️🎉️

One last problem: No matter what we do, we get an empty new to-do. Hey, that rhymes! If creating to-do apps doesn't work out, we could always try and be a poet.
...
We're telling TypeScript we're expecting a method that accepts an event as a parameter and doesn't return anyting; `void`. After you've updated `TodoForm` it should look like this:
...
and in `src/components/TodoForm.tsx` add our new `inputValue` property:
...
That's it for this tutorial. I hope I was able to convince you to use Typescript if you weren't sure yet and that you've learned how to get started with using TypeScript and React together. If you're up for a challenge you can try and implement functionality to finish a to-do or disabling the create button if there is no text inside the to-do input. Good luck!
